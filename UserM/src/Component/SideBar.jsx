...
const SideBar = ({
  ...
  /*
   * FIX tombol "Keluar" di sidebar:
   *
   * - Kalau saat login user MENCENTANG "Ingat saya 30 hari"
   *   (rememberMe === "true" di localStorage), maka "Keluar" dari
   *   sidebar TIDAK menghapus sesi. Kita cuma arahkan ke /login,
   *   yang otomatis akan redirect balik ke /dashboard karena
   *   isLoggedIn() masih true (lihat Login.jsx). Jadi sesuai
   *   permintaan: remember-me baru "kepakai" (tetap login) kalau
   *   sebelumnya memang dicentang.
   *
   * - Kalau remember-me TIDAK dicentang, "Keluar" dari sidebar
   *   berlaku seperti logout biasa: sesi dihapus total, wajib
   *   login ulang.
   *
   * Catatan: tombol "Logout" terpisah di halaman Profile
   * (Profile.jsx -> handleLogout) TETAP logout total tanpa syarat,
